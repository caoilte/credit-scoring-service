...
class MockCSCardsServer(server: WireMockServer, basePath: String) {
  ...
  def mockCardsResponse(httpStatus: Int, requestBody: Json, resource: String): StubMapping =
    server.stubFor(
      post(urlPathMatching(s"$basePath/v1/cards"))
  ...
  def mockCardsResponseWithDelay(httpStatus: Int, requestBody: Json, resource: String): StubMapping =
    server.stubFor(
      post(urlPathMatching(s"$basePath/v1/cards"))
        .withRequestBody(equalToJson(requestBody.toString()))
        .willReturn(
